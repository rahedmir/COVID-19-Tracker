...
import rahedmir.github.covid_19tracker.CountryModel;
...
public class myCustomAdapter extends ArrayAdapter<CountryModel> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
       ...
       View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.list_custom_item,null,true);
        TextView tvCountryName=view.findViewById(R.id.tvCountryNmae);
        ImageView imgView=view.findViewById(R.id.imgFlags);
        tvCountryName.setText(countryModelListFiltered.get(position).getCountry());
        Glide.with(context).load(countryModelListFiltered.get(position).getFlag()).into(imgView);
  ...
  MainActivity.AffectedCountries.countryModelList=(List<CountryModel>)results.values;
